/**
 * Protein Streak Embed Page
 *
 * Public embed page for protein goal streak counter.
 */
...
import { calculateStreak } from '../../utils/dashboardData';
import '../../components/Dashboard.css';

export default function ProteinStreakEmbed() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const data = await fetchEmbedData(token, 'protein-streak');
  ...
  return (
    <EmbedLayout title="Protein Goal Streak">
      <div className="progress-card" style={{ width: '100%', maxWidth: '500px', margin: '0 auto' }}>
        <div className="progress-header">
          ...
          <span className="progress-value">
            {streak > 0 ? 'Keep it up!' : 'Start logging daily'}
          </span>
        </div>
